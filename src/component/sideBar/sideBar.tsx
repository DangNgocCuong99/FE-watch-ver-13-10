          ...
          if (sideBar === false) {
            return children.map((item,key) => (
              <li className="leve12" key={key}>
                <a onClick={()=>navigation(item.href)} title={item.title}>
                  {item.content}
                </a>
              </li>
            ));
            ...
            return open ? children.map((item,key) => (
                <li key={key}>
                  <a  title={item.title} className="a3" onClick={()=>navigation(item.href)}>
                    {item.content}
                  </a>
                </li>
              )) : null
          ...
          if (sideBar === false) {
            ...
            return children.map((item,key) => (
              <li
                ...
                key={key}
              >
                <a className="hmega" onClick={()=>navigation(item.href)} title={item.title}>
                  {item.content}
                </a>
                <ul className="level1">
            ...
            return open ?
               children.map((item,key) => (
                <li className="current" key={key}>
                  <a className="caret-down" onClick={()=>navigation(item.href)} title={item.title}>
                    {item.content}
                  </a>
        ...
        if (item.type === "item") {
          return (
            <li className="nav-item" key={key}>
              <a  onClick={()=>navigation(item.href)} className="a-img" title={item.title}>
                {item.title}
              </a>
            </li>
          );
          ...
          return (
            <li className="nav-item has-mega current" key={key}>
              <a  onClick={()=>navigation(item.href)} className="a-img caret-down" title={item.title}>
                {item.content}
              </a>
